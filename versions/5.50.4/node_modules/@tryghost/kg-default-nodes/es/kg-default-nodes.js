...
function renderHeaderNodeToDOM(node) {
  let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  addCreateDocumentOption(options);
  const document = options.createDocument();
...
const INSERT_HEADER_COMMAND = createCommand();
...
function renderGalleryNodeToDOM(node) {
  let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  addCreateDocumentOption(options);
  const document = options.createDocument();
...
const INSERT_GALLERY_COMMAND = createCommand();
...
function renderEmailCtaNodeToDOM(node) {
  let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  addCreateDocumentOption(options);
  const document = options.createDocument();
  const html = node.getHtml();
...
const INSERT_EMAIL_CTA_COMMAND = createCommand();
        ...
        if (nodeElem.tagName === 'DIV' && isSignupNode) {
          return {
            conversion(domNode) {
              ...
              const backgroundImageSrc = layout === 'split' ? domNode.querySelector('.kg-signup-card-image')?.getAttribute('src') : domNode.querySelector('.kg-signup-card-container')?.getAttribute('style')?.match(/url\((.+)\)/)?.[1] || '';
...
const INSERT_SIGNUP_COMMAND = createCommand();
...
class SignupNode extends KoenigDecoratorNode {
  ...
  isEmpty() {
    return !this.__backgroundColor && !this.__backgroundImageSrc && !this.__buttonColor && !this.__buttonText && !this.__disclaimer && !this.__header && !this.__labels.length && !this.__subheader && !this.__successMessage;
  }
}
...
const DEFAULT_NODES = [CodeBlockNode, ImageNode, MarkdownNode, VideoNode, AudioNode, CalloutNode, AsideNode, HorizontalRuleNode, HtmlNode, FileNode, ToggleNode, ButtonNode, HeaderNode, BookmarkNode, PaywallNode, ProductNode, EmbedNode, EmailNode, GalleryNode, EmailCtaNode, SignupNode];

export { $createAsideNode, $createAudioNode, $createBookmarkNode, $createButtonNode, $createCalloutNode, $createCodeBlockNode, $createEmailCtaNode, $createEmailNode, $createEmbedNode, $createFileNode, $createGalleryNode, $createHeaderNode, $createHorizontalRuleNode, $createHtmlNode, $createImageNode, $createMarkdownNode, $createPaywallNode, $createProductNode, $createSignupNode, $createToggleNode, $createVideoNode, $isAsideNode, $isAudioNode, $isBookmarkNode, $isButtonNode, $isCalloutNode, $isCodeBlockNode, $isEmailCtaNode, $isEmailNode, $isEmbedNode, $isFileNode, $isGalleryNode, $isHeaderNode, $isHorizontalRuleNode, $isHtmlNode, $isImageNode, $isKoenigCard, $isMarkdownNode, $isPaywallNode, $isProductNode, $isSignupNode, $isToggleNode, $isVideoNode, AsideNode, AudioNode, BookmarkNode, ButtonNode, CalloutNode, CodeBlockNode, DEFAULT_NODES, EmailCtaNode, EmailNode, EmbedNode, FileNode, GalleryNode, HeaderNode, HorizontalRuleNode, HtmlNode, INSERT_AUDIO_COMMAND, INSERT_BOOKMARK_COMMAND, INSERT_BUTTON_COMMAND, INSERT_CALLOUT_COMMAND, INSERT_CODE_BLOCK_COMMAND, INSERT_EMAIL_COMMAND, INSERT_EMAIL_CTA_COMMAND, INSERT_EMBED_COMMAND, INSERT_FILE_COMMAND, INSERT_GALLERY_COMMAND, INSERT_HEADER_COMMAND, INSERT_HORIZONTAL_RULE_COMMAND, INSERT_HTML_COMMAND, INSERT_IMAGE_COMMAND, INSERT_MARKDOWN_COMMAND, INSERT_PAYWALL_COMMAND, INSERT_PRODUCT_COMMAND, INSERT_SIGNUP_COMMAND, INSERT_TOGGLE_COMMAND, INSERT_VIDEO_COMMAND, ImageNode, ImageParser, KoenigDecoratorNode, MarkdownNode, PaywallNode, ProductNode, SignupNode, ToggleNode, UPLOAD_IMAGE_COMMAND, VideoNode, bytesToSize };
